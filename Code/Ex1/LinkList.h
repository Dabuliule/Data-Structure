...
//  单链表的函数成员 
	...
	LinkList(int n, ElemType val);		// 构造一个长度为 n 值为 val 的单链表
...
template <class ElemType>
LinkList<ElemType>::LinkList(int n, ElemType val)
// 操作结果：构造了一个长度为 n，值都为 val 的单链表
{
	Node<ElemType> *p;				// 构造头结点 
	p = head = new Node<ElemType>;	// 构造头结点失败，终止程序运行
	assert(head != 0);
	for (int i = 0; i < n; ++i) {
		p->next = new Node<ElemType>(val, NULL);
		assert(p->next);			// 构造元素结点失败，终止程序运行
		p =  p->next;
	}
	length = n;						// 初始化单链表长度为n
