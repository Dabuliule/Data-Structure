...
// 顺序表的函数成员
	...
	Status GetMax(ElemType &e) const;					// 得到最大值
	Status GetMin(ElemType &e) const;					// 得到最小值
...
template <class ElemType>
Status SeqList<ElemType>::GetMax(ElemType &e) const
// 功能：用 e 返回当前顺序表的最大值
{
	if(IsEmpty()){
		return EMPTY;		// 顺序表为空，返回 EMPTY
	}
	e = elems[0];			// 用 e 记录最大值
	for(int i = 1; i < GetLength(); ++i){
		if(e < elems[i]){	// 如果 e 更小，就更新
			e = elems[i];
		}
	}
	return SUCCESS;			// 返回成功得到最大值
}

template <class ElemType>
Status SeqList<ElemType>::GetMin(ElemType &e) const
// 功能：用 e 返回当前线性表的最小值
{
	if(IsEmpty()){
		return EMPTY;		// 顺序表为空，返回 EMPTY
	...
	e = elems[0];			// 用 e 记录最小值
	for(int i = 1; i < GetLength(); ++i){
		if(e > elems[i]){	// 如果 e 更大，就更新
			e = elems[i];
		}
	}
	return SUCCESS;			// 返回成功得到最小值
}
